...
def sentiment():
    ...
    result = {}
    if key == country:
        response = requests.get('http://{}:5984/twitter/_design/{}/_view/{}?group=True&group_level=1'.format(address, 'place_query', key))
        for row in response.json()['rows']:
            if row['key'] in state:
                result[row['key']] = row['value']['sum'] / row['value']['count']
    elif key in state:
        response = requests.get('http://{}:5984/twitter/_design/{}/_view/{}?group=True'.format(address, 'place_query', key))
        for row in response.json()['rows']:
